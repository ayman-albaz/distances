...
func jaccardDistance*[T: SomeNumber](x1, x2: openArray[T]): float =
  ...
  ## When both arrays contain only zeros, the distance is `0.0`.
  checkSameLength(x1, x2)
  let n = x1.len
  var totalMin = 0.0
  var totalMax = 0.0
...
func cosineDistance*[T: SomeNumber](x1, x2: openArray[T]): float =
  ...
  ## If either vector has zero magnitude, the result is `NaN` since cosine
...
func klDivergenceDistance*[T: SomeNumber](x1, x2: openArray[T]): float =
  ...
  ##
  ## Returns `NaN` if any element is negative (not a valid probability
  ## distribution). Returns `Inf` if `x1[k] > 0` and `x2[k] == 0`.
  ## Returns `0.0` for empty arrays.
  checkSameLength(x1, x2)
  let n = x1.len
  var total = 0.0
...
func pairwise*[T](
    X: openArray[seq[T]], distance: (proc(x1, x2: openArray[T]): float {.noSideEffect.})
): seq[seq[float]] =
...
proc pairwise*[T](
    dst: var seq[seq[float]],
    X: openArray[seq[T]],
    distance: (proc(x1, x2: openArray[T]): float {.noSideEffect.}),
) =
...
func symmetrize*[T](
    X: openArray[seq[T]], how: SymmetrizeDir = sdLowerToUpper
): seq[seq[T]] =
